        ...
        <div className="flex rounded-lg flex-col w-full shadow-secondary lg:rounded-2xl lg:flex-row">
          <div className="left-top-box py-8 px-7 bg-white lg:max-w-[43%] md:px-12 lg:px-[80px]">
            <div className="flex flex-col justify-center h-full text-center lg:text-left">
              <h2 className="text-[32px] font-semibold tracking-[-0.01em] leading-[38px] md:text-[44px] md:leading-[52px]">
                Fastest Growing{" "}
          ...
          <div className="right-bottom-box bg-[#D3ECF7] bg-opacity-70 w-full px-4 pt-[48px] pb-[16px] sm:pt-[84px] sm:pb-[84px] sm:px-10 xl:px-[68px]">
            <div className="grid gap-3 sm:grid-cols-2">
              {ACHIEVEMENT_LIST.map((item) => (
                <div
                  key={item.name}
                  className="bg-white rounded flex flex-col gap-1 lg:gap-[2px] items-center justify-center text-center py-5 min-h-[40px] sm:rounded-md"
                >
                  <Image
                    src={item.icon}
                    alt={item.name}
